function [a_red_anal,a_red_num,a_red_S3L] = analytical_propagator03(a_drag,a_S3L, tspan) 
...
%   - a_S3          : Semi-major axis computed with the S3L propagator [m]
%   - tspan         : Vector of time properties        

% OUTPUTS
%   - a_red_anal    : Reduction of the semi-major axis of the orbit [m] (analytical)
%   - a_red_num     : Reduction of the semi-major axis of the orbit [m] (numerical - propagator03)
%   - a_red_S3L     : Reduction of the semi-major axis of the orbit [m] (numerical - S3L propagator)
...
t_f = tspan(end);               % Final time (24h) [s]
...
a_red_anal = diff;      % Difference between the initial and final semi-major axes [m] (OUTPUT)

%% Computation of the Reduction in the Semi-Major Axis (Propagator03)

diff = a_drag(end,1) - a_i;       % Difference between the initial and final semi-major axes [m]

a_red_num = diff;           % Difference between the initial and final semi-major axes [m] (OUTPUT)

%% Computation of the Reduction in the Semi-Major Axis (S3L Propagator)

diff = a_S3L(end,1) - a_i;        % Difference between the initial and final semi-major axes [m]

a_red_S3L = diff;           % Difference between the initial and final semi-major axes [m] (OUTPUT)

%% Plot

plot([t_i,t_f/3600],[a_i/1000,a_f/1000],'color',[252 186 3]/255,'LineWidth',1.5)
hold on
plot([t_i,t_f/3600],[a_i/1000,a_drag(end,1)/1000],'color',[15 43 184]/255,'LineWidth',1.5)
hold on
plot([t_i,t_f/3600],[a_i/1000,a_S3L(end,1)/1000],'color',[201 4 4]/255,'LineWidth',1.5)
h=legend('Analytical result','DF3d propagator (drag only)','S3L propagator (drag only)');
set(h,'Location','south west');
grid minor
xlabel('Time [h]','Fontsize',12)
ylabel('Semi-Major Axis [km]','Fontsize',12)
